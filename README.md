...
- 每轮候选扫描窗口可独立配置；当前运行版使用 200 条窗口，更快回到类目刷新而不改变选品门槛。
- 提供跨店软去重辅助模块：同一店铺永不重复提交；只有其他店铺已成功的商品，才按店铺与商品稳定抽取 20% 复用；失败商品不会复用。
- 共享商品占用采用原子文件锁，多个店铺并发时同一商品同时只能有一个提交者。
- 汇总状态同时报告运行、额度受限和候选队列等待的店铺数，适合多店铺看板直接读取。
- 不依赖浏览器；真正的平台或 ERP 调用由外部引擎适配器实现。

## 当前运行版同步

本仓库的 `v1.3` 调度逻辑已按 2026-08-28 正在运行的 Flow E/F 同步，包含：十主店分片、备用店接管、平台额度权威判断、非零点额度重置、共享候选池单领导者、启动错峰、接口限流退避、安全停止、200 条候选刷新窗口，以及成功商品跨店 20% 软复用与原子防撞。
...
| `FLOWDE_MAX_SCANS_PER_CYCLE` | 每轮候选扫描窗口；当前运行版默认 200 |
| `FLOWDE_CROSS_STORE_REUSE_DIVISOR` | 成功商品跨店稳定抽样分母；默认 5，即约 20% |
| `FLOWDE_SHARED_CLAIM_DIR` | 多店共享的商品原子占用目录 |
...
`src/lib/item-claims.mjs` 提供 `duplicateDisposition`、`acquireItemClaim`、`updateItemClaim` 和 `releaseItemClaim`。适配器应先读取跨店审计记录并调用 `duplicateDisposition`：

- 当前店已尝试过：始终拦截。
- 其他店成功过：仅当返回 `reuse` 时允许复用，默认是稳定的 20% 商品/店铺组合。
- 其他店只有失败记录：始终拦截，不把失败商品扩散到更多店铺。
- 无历史记录：按新商品处理。

进入正式提交前再调用 `acquireItemClaim`，成功后将占用更新为 `completed`；尚未提交就退出时释放占用。这样可以防止两个并发店铺在同一时刻提交同一商品。该模块只保存商品占用状态，不包含商品详情或平台凭证。
